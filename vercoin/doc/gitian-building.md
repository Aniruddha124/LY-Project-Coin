...
*Setup instructions for a Gitian build of Vercoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Vercoin
...
- [Building Vercoin Core](#building-vercoin-core)
...
Clone the git repositories for vercoin and Gitian.
...
in the vercoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Vercoin Core
----------------

To build Vercoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the vercoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vercoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/vercoin-project/vercoin
...
For example:
```bash
URL=https://github.com/thrasher-/vercoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit vercoin=${COMMIT} --url vercoin=${URL} ../vercoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit vercoin=${COMMIT} --url vercoin=${URL} ../vercoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit vercoin=${COMMIT} --url vercoin=${URL} ../vercoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the vercoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../vercoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/vercoin
SIGPATH=/some/root/path/vercoin-detached-sigs

./bin/gbuild --url vercoin=${BTCPATH},signature=${SIGPATH} ../vercoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vercoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vercoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/vercoin-osx-build.assert
```
...
[vercoin-project/gitian.sigs.ltc](https://github.com/vercoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
