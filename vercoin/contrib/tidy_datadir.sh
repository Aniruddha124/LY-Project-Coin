...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Vercoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Vercoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Vercoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Vercoin 0.7 datadir."
    ;;
  3)
    echo "Detected Vercoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Vercoin 0.8 datadir."
    ;;
esac
